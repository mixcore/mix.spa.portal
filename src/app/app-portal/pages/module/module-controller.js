...
app.controller("ModuleController", [
  ...
  function($scope, $rootScope, ngAppSettings, $location, $routeParams,
           moduleServices, moduleDataService,
           RestRelatedMixDatabasePortalService, dataService) {
    BaseRestCtrl.call(this, $scope, $rootScope, $location, $routeParams,
                      ngAppSettings, moduleServices, "product");
    $scope.contentUrl = "";
    $scope.getSingleSuccessCallback = function() {
      ...
      if ($scope.viewModel.sysCategories) {
        angular.forEach($scope.viewModel.sysCategories,
                        function(e) { e.attributeData.obj.isActived = true; });
      }

      if ($scope.viewModel.sysTags) {
        angular.forEach($scope.viewModel.sysTags,
                        function(e) { e.attributeData.obj.isActived = true; });
      }
    ...
    $scope.removeData = function(id) {
      if ($scope.viewModel) {
        $rootScope.showConfirm(
            $scope, "removeDataConfirmed", [ id ], null, "Remove Data",
    ...
    $scope.updateModuleDataField = async function(item, propertyName) {
      var result = await moduleDataService.saveFields(item.id, propertyName,
                                                      item[propertyName]);
      if (result.isSucceed) {
        $scope.loadModuleDatas();
      } else {
    ...
    $scope.saveSuccessCallback = async function() {
      if ($scope.additionalData) {
        ...
        var saveData =
            await dataService.saveAdditionalData($scope.additionalData);
    ...
    $scope.removeAttribute = function(attr, index) {
      $rootScope.showConfirm(
          $scope, "removeAttributeConfirmed", [ attr, index ], null,
          "Remove Field",
          "Deleted data will not able to recover, are you sure you want to delete this item?");
    };
