...
app.controller("MixDatabaseDataController", [
  ...
  function($scope, $rootScope, ngAppSettings, $routeParams, $location, service,
    ...
    $scope.initRouteParams = () => {
      $scope.requestKey =
          `request${$rootScope.generateKeyword($location.$$path, "_")}_${
              $routeParams.mixDatabaseId}`;
    ...
    $scope.init = async function() {
      ...
      if ($scope.mixDatabaseName || $scope.mixDatabaseId) {
        var getFields = await columnService.initData($scope.mixDatabaseName ||
                                                     $scope.mixDatabaseId);
    ...
    $scope.saveSuccess = function(data) { $scope.viewmodel = data; };
