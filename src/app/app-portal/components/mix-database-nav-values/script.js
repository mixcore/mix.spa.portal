modules.component("mixDatabaseNavValues", {
  ...
  controller : [
    ...
    function($rootScope, $scope, ngAppSettings, navService, fieldService) {
      ...
      ctrl.update = function(data) { ctrl.onUpdate({data : data}); };

      ctrl.delete = function(data) { ctrl.onDelete({data : data}); };

      ctrl.filterData = function(item, attributeName) {
        return $rootScope.findObjectByKey(item.data, "attributeName",
                                          attributeName);
      };
      ...
      ctrl.updateData = function(nav) {
        $rootScope.goToPath(`${ctrl.updateUrl}?dataId=${
            nav.dataId}&mixDatabaseId=${nav.mixDatabaseId}&parentId=${
            ctrl.parentId}&parentType=${ctrl.parentType}`);
            ...
            navService.save("portal", ctrl.refDataModel).then((resp) => {
              if (resp.isSucceed) {
                var tmp = $rootScope.findObjectByKey(
                    ctrl.refData, [ "parentId", "parentType", "id" ],
                ...
                var e =
                    $(".pane-data-" + ctrl.attributeValue.field.referenceId)[0];
                angular.element(e).triggerHandler("click");
                $rootScope.isBusy = false;
                $scope.$apply();
              } else {
