modules.component("mixDatabaseDataValues", {
  ...
  controller : [
    ...
    function($rootScope, $scope, fieldService, dataService) {
      ...
      ctrl.select = function(item) {
        if (item.isSelected) {
          if (ctrl.selectSingle == "true") {
            ...
            var current =
                $rootScope.findObjectByKey(ctrl.selectedList, "id", item.id);
            if (!current) {
              ctrl.selectedList.data.push(item);
            }
          }
      ...
      ctrl.sendMail = async function(data) { ctrl.onSendMail({data : data}); };
      ctrl.apply = async function() { ctrl.onApplyList(); };

      ctrl.duplicate = function(data) { ctrl.onDuplicate({data : data}); };
