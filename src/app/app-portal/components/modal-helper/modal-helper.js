modules.component("modalHelper", {
  ...
  controller : [
    ...
    "$location",
    "$sce",
    function($rootScope, $scope, localStorageService, $routeParams, $location,
             $sce) {
      var ctrl = this;
      ...
      ctrl.loadHelperUrl = function() {
        ...
        if (portalUrl.startsWith(
                "/portal/mix-database-data/list?mixDatabaseId=2")) {
          ctrl.trustedUrl =
              $sce.trustAsResourceUrl(defaultUrl + "/basic-usage-navigation");
        }
        ...
        if (portalUrl.startsWith("/portal/page")) {
          ctrl.trustedUrl =
              $sce.trustAsResourceUrl(defaultUrl + "/basic-usage-page");
        }
        if (portalUrl.startsWith("/portal/module")) {
          ctrl.trustedUrl =
              $sce.trustAsResourceUrl(defaultUrl + "/basic-usage-module");
        }
        if (portalUrl.startsWith("/portal/mix-database")) {
          ctrl.trustedUrl =
              $sce.trustAsResourceUrl(defaultUrl + "/basic-usage-database");
        }
        if (portalUrl.startsWith(
                "/portal/mix-database-data/list?mixDatabaseId=7")) {
          ctrl.trustedUrl =
              $sce.trustAsResourceUrl(defaultUrl + "/basic-usage-category");
        }
        if (portalUrl.startsWith(
                "/portal/mix-database-data/list?mixDatabaseId=8")) {
          ctrl.trustedUrl =
              $sce.trustAsResourceUrl(defaultUrl + "/basic-usage-tag");
        }
        if (portalUrl.startsWith("/portal/media")) {
          ctrl.trustedUrl =
              $sce.trustAsResourceUrl(defaultUrl + "/basic-usage-media");
        }
        if (portalUrl.startsWith("/portal/file")) {
          ctrl.trustedUrl =
              $sce.trustAsResourceUrl(defaultUrl + "/basic-usage-file");
        }
        if (portalUrl.startsWith("/portal/user")) {
          ctrl.trustedUrl =
              $sce.trustAsResourceUrl(defaultUrl + "/basic-usage-user");
        }
        if (portalUrl.startsWith("/portal/theme")) {
          ctrl.trustedUrl =
              $sce.trustAsResourceUrl(defaultUrl + "/basic-usage-theme");
        }
        if (portalUrl.startsWith("/portal/app-settings")) {
          ctrl.trustedUrl =
              $sce.trustAsResourceUrl(defaultUrl + "/basic-usage-app-settings");
        }
