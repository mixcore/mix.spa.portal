  ...
  controller : [
    ...
    function($rootScope, $scope, $uibModal, ngAppSettings, mediaService) {
      ...
      ctrl.$onInit = function() {
        ...
        ctrl.isImage = ctrl.srcUrl.toLowerCase().match(
      ...
      ctrl.croppieCallback = function(
          result) { ctrl.srcUrl = result.filePath; };
      ...
      ctrl.uploadFile = async function(file) {
        if (file !== null) {
          ...
          if (getMedia.isSucceed) {
            ctrl.mediaFile.fileName =
                file.name.substring(0, file.name.lastIndexOf("."));
            ctrl.mediaFile.extension =
            ...
            var resp =
                await mediaService.save(media, null, ctrl.onUploadFileProgress);
      ...
      ctrl.getBase64 = function(file) {
        if (file !== null) {
          ...
          reader.onload = function() {
            ...
            if (ctrl.postedFile) {
              ctrl.postedFile.fileName =
                  file.name.substring(0, file.name.lastIndexOf("."));
              ctrl.postedFile.extension =
                  file.name.substring(file.name.lastIndexOf("."));
              ctrl.postedFile.fileStream = reader.result;
            }
            ctrl.srcUrl = reader.result;
            ctrl.isImage = ctrl.srcUrl.indexOf("data:image/") >= 0 ||
                           ctrl.srcUrl.toLowerCase().match(
                               /([/|.|\w|\s|-])*\.(?:jpg|jpeg|gif|png|svg)/g);
      ...
      ctrl.onUploadFileProgress = function(
          progress) { ctrl.progress = progress; };
    },
  ],
});
