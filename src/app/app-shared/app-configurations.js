...
appShared.run([
  ...
  "AuthService",
  "TranslatorService",
  function($http, $rootScope, ngAppSettings, $location, baseRestService,
           apiService, commonService, authService, translatorService) {
    ...
    $rootScope.generateKeyword = function(src, character, isCamelCase = false,
      ...
      if (src) {
        ...
        if (isCamelCase) {
          src = src.replace(/\w\S*/g, function(txt) {
          ...
          if (isLowerFirst) {
            src = src.replace(/^./,
                              (str) => str.toLowerCase()); // first to lowercase
          }
        } else {
          src = src.replace(/[^a-zA-Z0-9]+/g, character)
    ...
    $rootScope.updateSettings = function() {
      ...
      commonService.fillSettings($rootScope.globalSettings.lang)
          .then(function(response) { $rootScope.globalSettings = response; });
      $rootScope.isBusy = false;
    };
    $rootScope.executeFunctionByName =
        async function(functionName, args, context = window) {
    ...
    $rootScope.showConfirm = function(context, okFuncName, okArgs,
                                      cancelFuncName, title, msg, cancelArgs,
                                      lblOK, lblCancel) {
    ...
    $rootScope.showErrors = function(errors) {
      if (errors.length) {
        $.each(errors, function(i, e) { $rootScope.showMessage(e, "danger"); });
      } else {
    ...
    $rootScope.showMessage = function(content, type) {
      // $rootScope.toast = new
      // bootstrap.Toast(document.getElementById('toast-msg'));
    ...
    $rootScope.getConfiguration = function(keyword, isWrap, defaultText) {
      if ($rootScope.globalSettings &&
        ...
        return $rootScope.globalSettingsService.get(keyword, isWrap,
                                                    defaultText);
      } else {
        return keyword || defaultText;
      }
      ...
      if (array) {
        if (Array.isArray(key)) {
          for (var i = 0; i < array.length; i++) {
            ...
            angular.forEach(
                key,
                function(e, j) { isMatch = array[i][key[j]] == value[j]; });
            if (isMatch) {
              return array[i];
            }
          }
    ...
    $rootScope.removeObjectByKey = function(array, key, value) {
      if (Array.isArray(key)) {
        for (var i = 0; i < array.length; i++) {
          var isMatch = true;
          angular.forEach(
              key, function(e, j) { isMatch = array[i][key[j]] == value[j]; });
          if (isMatch) {
            array.splice(i, 1);
            break;
          }
    ...
    $rootScope.goToSiteUrl = function(url) { window.top.location = url; };
    $rootScope.goToPath = function(url) { $location.url(url.trim()); };
    $rootScope.encryptMixDatabase = function(attributes, data) {
      angular.forEach(attributes, function(attr) {
        if (attr.isEncrypt) {
          angular.forEach(data, function(item) {
            var columnData = $rootScope.findObjectByKey(
                item.data, "attributeName", attr.name);
            if (columnData) {
    ...
    $rootScope.isInRole = function(
        roleName) { return authService.isInRole(roleName); };
    ...
    $rootScope.openModal = function(templateUrl, controllerName, resolve,
                                    size = "lg", successCallback = null,
                                    failCallback = null) {
      ...
      modalInstance.result.then(function(result) { successCallback(result); },
                                function(error) { failCallback(error); });
    };

    $rootScope.isImage = function(file) {
      let ext = file.name.substring(file.name.lastIndexOf("."));
      return ext.toLowerCase().match(
