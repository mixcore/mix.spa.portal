  ...
  controller : [
    ...
    function($rootScope, $scope, $location, ngAppSettings, $filter,
      ...
      ctrl.$doCheck = function() {
        ...
        if (ctrl.model && (!ctrl.model.id || !ctrl.model.obj["seo_url"]) &&
        ...
        // check encrypt data
        if (ctrl.model && ctrl.field && ctrl.field.isEncrypt &&
            ctrl.model.obj[ctrl.field.name] &&
            $rootScope.testJSON(ctrl.model.obj[ctrl.field.name])) {
          ctrl.model.obj[ctrl.field.name] =
              ctrl.parseEncryptedData(ctrl.model.obj[ctrl.field.name]);
        }
      }.bind(ctrl);
      ctrl.refData = null;
      ...
      ctrl.$onInit = function() {
        if (!ctrl.createUrl && ctrl.model && ctrl.field.referenceId) {
          ...
          ctrl.createUrl = `/portal/mix-database-data/create?mixDatabaseId=${
              ctrl.field.referenceId}&dataId=default&parentId=${
              ctrl.model.id}&parentType=Set&backUrl=${backUrl}`;
        }
        if (!ctrl.updateUrl) {
      ...
      ctrl.initData = async function() {
        setTimeout(() => {
          ...
          case "time":
            if (ctrl.model.obj[ctrl.field.name]) {
              var local =
                  $filter("utcToLocalTime")(ctrl.model.obj[ctrl.field.name]);
