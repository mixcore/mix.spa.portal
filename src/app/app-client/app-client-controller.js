...
app.controller("AppClientController", [
  ...
  function($rootScope, $scope, globalSettingsService, commonService,
           authService, localStorageService, translatorService,
           moduleDataService, attrDataService) {
    $scope.lang = "";
    $scope.isInit = false;
    ...
    $scope.init = function(lang) {
      ...
      //     if(!$(this).hasClass('each-portfolio') && href && href.indexOf('#')
      //     !== 0 && target!='_blank'){
      //         e.preventDefault();
    ...
    $scope.initModuleForm =
        async function(name, successCallback, failCallback) {
      var resp = null;
      ...
      setTimeout(async () => {
        $scope.name = name;
        if ($scope.id) {
          resp = await moduleDataService.getModuleData($scope.id, $scope.dataId,
                                                       "portal");
        } else {
        ...
        if (resp && resp.isSucceed) {
          ...
          if (resp) {
            if ($scope.errorCallback) {
              $rootScope.executeFunctionByName($scope.errorCallback, [ resp ],
                                               window);
            } else {
    ...
    $scope.initMixDatabaseForm = async function(
        formName) { return await attrDataService.initData(formName).data; };

    $scope.saveModuleData = async function() {
      var resp =
          await moduleDataService.saveModuleData($scope.activedModuleData);
      if (resp && resp.isSucceed) {
        $scope.activedModuleData = resp.data;
        if ($scope.successCallback) {
          $rootScope.executeFunctionByName($scope.successCallback, [ resp ],
                                           window);
        } else {
          var msg =
        ...
        if (resp) {
          if ($scope.failCallback) {
            $rootScope.executeFunctionByName($scope.failCallback, [ resp ],
                                             window);
          } else {
            $rootScope.showErrors(resp.errors);
          }
    ...
    window.load = function() { $scope.$apply(($scope.isLoaded = true)); };
  },
]);
})(window.angular);
