...
var gulp = require("gulp"), rimraf = require("rimraf"),
    concat = require("gulp-concat"), cssmin = require("gulp-cssmin"),
    // fontmin = require("gulp-fontmin"),
    htmlmin = require("gulp-htmlmin"),
    removeHtmlComments = require("gulp-remove-html-comments"),
    header = require("gulp-header"), uglify = require("gulp-uglify-es").default;
...
var paths = {
  ...
  webroot :
      "../mix.core/src/Mix.Cms.Web/wwwroot/mix-app/", // Use for mix.core repo
  webapp : "./src/app/",                              // app
  libs : "./src/lib/",
...
gulp.task("clean:portalApp",
          function(cb) { rimraf(paths.appPortal.dest, cb); });
...
gulp.task("clean:portalAppRequired",
          function(cb) { rimraf(paths.appPortalRequired.dest, cb); });
...
gulp.task("clean:portalCss",
          function(cb) { rimraf(paths.portalCss.dest, cb); });
...
gulp.task("clean:shared", function(cb) { rimraf(paths.shared.dest, cb); });

gulp.task("clean:framework",
          function(cb) { rimraf(paths.framework.dest, cb); });

gulp.task("clean:clientApp",
          function(cb) { rimraf(paths.clientApp.dest, cb); });
gulp.task("clean:clientAppRequired",
          function(cb) { rimraf(paths.clientAppRequired.dest, cb); });

gulp.task("clean:sharedApp",
          function(cb) { rimraf(paths.sharedApp.dest, cb); });

gulp.task("clean:initApp", function(cb) { rimraf(paths.initApp.dest, cb); });

gulp.task("clean:securityApp",
          function(cb) { rimraf(paths.securityApp.dest, cb); });

gulp.task("clean:appCss", function(cb) { rimraf(paths.appCss.dest, cb); });

gulp.task("clean:appInitCss",
          function(cb) { rimraf(paths.appInitCss.dest, cb); });

gulp.task("clean:sharedCss",
          function(cb) { rimraf(paths.sharedCss.dest, cb); });

gulp.task("clean:css", gulp.series("clean:appCss", "clean:appInitCss",
                                   "clean:portalCss", "clean:sharedCss"));
gulp.task("clean:js",
          gulp.series("clean:framework", "clean:shared", "clean:portalApp",
                      "clean:portalAppRequired", "clean:clientApp",
                      "clean:clientAppRequired", "clean:sharedApp",
                      "clean:initApp", "clean:securityApp"));
gulp.task("min:js",
          gulp.series("min:portalApp", "min:portalAppRequired", "min:initApp",
                      "min:securityApp", "min:clientApp",
                      "min:clientAppRequired", "min:sharedApp", "min:shared",
                      "min:framework"));
gulp.task("min:css",
          gulp.series("min:scss", "min:appCss", "min:appClientCss",
                      "min:appInitCss", "min:portalCss", "min:sharedCss"));
...
gulp.task("buildp",
          gulp.series("min:portalApp", "min:scss", "min:appCss",
                      "min:appClientCss", "min:portalCss", "min:views"));
gulp.task("watch", function() {
  gulp.watch("./src/app/**/**/*.html", gulp.series("min:views"));
  gulp.watch("./src/app/app-portal/**/*.js", gulp.series("min:portalApp"));
  gulp.watch("./src/app/app-portal/shared/**/*.js",
  ...
  gulp.watch("./src/app/app-client/**/*.js",
